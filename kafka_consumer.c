...
static void consumer_commit(int async, INTERNAL_FUNCTION_PARAMETERS) /* {{{ */
{
    ...
    if (zarg) {
        if (Z_TYPE_P(zarg) == IS_OBJECT && instanceof_function(Z_OBJCE_P(zarg), ce_kafka_message TSRMLS_CC)) {
            ...
            zerr = zend_read_property(NULL, zarg, ZEND_STRL("err"), 0, &rv TSRMLS_CC);
            ...
            ztopic = zend_read_property(NULL, zarg, ZEND_STRL("topic_name"), 0, &rv TSRMLS_CC);
            ...
            zpartition = zend_read_property(NULL, zarg, ZEND_STRL("partition"), 0, &rv TSRMLS_CC);
            ...
            zoffset = zend_read_property(NULL, zarg, ZEND_STRL("offset"), 0, &rv TSRMLS_CC);
